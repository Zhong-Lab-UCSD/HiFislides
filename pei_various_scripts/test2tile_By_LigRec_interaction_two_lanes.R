...
L = as.numeric(arg[which(arg == "--args") + 1])
...
data = read.table(gsub("=",L,"MatrixL00=_geneXtile.o"),sep="\t")
...
pdf(gsub("=",L,"pval_Li_Re_L=.pdf"));
if(L == 2) {
	plot(cex=2,x=-1*log10(resultp_1[x,9]),y=-1*log10(resultp_2[x,9]),pch=20,xlab="-log10(p-val) Ligand",ylab="-log10(p-val) Receptor",col="blue",main="Lane 2 (no tissue)");
}
if(L == 1) {
	x1 = setdiff(x,"Tile_1208")
	plot(x=-1*log10(resultp_1[x,9]),y=-1*log10(resultp_2[x,9]),pch=20,xlab="-log10(p-val) Ligand",ylab="-log10(p-val) Receptor",col="white",main="Lane 1 (with tissue)");
	points(cex=2,x=-1*log10(resultp_1[x1,9]),y=-1*log10(resultp_2[x1,9]),pch=20,col="blue");
	points(cex=2,x=-1*log10(resultp_1["Tile_1208",9]),y=-1*log10(resultp_2["Tile_1208",9]),pch=20,col="red");
}
dev.off()
