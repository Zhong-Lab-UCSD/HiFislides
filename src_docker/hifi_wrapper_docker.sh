...
usage() {
    cat << EOF >&2
    ---------------------------------------------------------------------------
    Usage: $PROGNAME [-i <star_index>] [-G <ref_fa>] [-g <ref_gtf>]
                     [-N <sample_name>]
                     [-S <spatial_barcode_dir>] [-f <flowcell_id>]
    ...
    -G : Reference genome fasta (optional if -i declared).
...
while getopts :i:G:g:N:S:f:1:2:t:o:h opt; do
    case $opt in
        i) STAR_INDEX=${OPTARG};;
...
echo "[$(date '+%m-%d-%y %H:%M:%S')] Generate STAR indexes..."
STAR_INDEX=$OUT_DIR/STARindex_withSJ
mkdir -p $STAR_INDEX
STAR \
--runThreadN $N_THREADS \
--runMode genomeGenerate \
--genomeDir $OUT_DIR/STARindex_withSJ \
--genomeFastaFiles $GENOME_FASTA \
--sjdbGTFfile $ANNOTATION_GTF_FILE
echo "[$(date '+%m-%d-%y %H:%M:%S')] STAR indexes generation done."

else 

echo "[$(date '+%m-%d-%y %H:%M:%S')] Error!! Please provide path to the STAR index files with -i or the reference genome fasta file with -G." && parameter_error
...
cat $L2R2_GENOME_DIR/HiFi_L2R2_genome_temp_1.bed $L2R2_GENOME_DIR/HiFi_L2R2_genome_temp_2.bed | sort -k 1 --parallel=$N_THREADS > $L2R2_GENOME_DIR/HiFi_L2R2_genome_ALL.sort.tsv
...
-k 19 \
-t $N_THREADS \
$L1R1_FASTQ_BWA_INDEX $L2R1_FASTQ 2>$L2_DIR/L2R1_mapping/L2R1_L1R1_dedup.log | grep -P "\t0\t$SEQ_MACHINE_ID|\t256\t$SEQ_MACHINE_ID" | awk -F"\t" 'NR==FNR{a[$1]; next} FNR==0 || $1 in a' $L2R2_GENOME_DIR/HiFi_L2R2_genome_ALL.sort.tsv - > $L2R1_MAPPING_DIR/L2R1_L1R1_dedup.filter.sam
...
python3 /usr/local/bin/hifiwrangling0.py \
$L2R1_MAPPING_DIR/L2R1_L1R1_dedup.filter.sam \
$L1R1_DUP \
1000 | sort -k 1 --parallel=$N_THREADS > $L2R1_MAPPING_DIR/L2R1_L1R1.hifiwrangling0.sort.o
...
### Genome
join -1 1 -2 1 -t $'\t' $L2R1_MAPPING_DIR/L2R1_L1R1.hifiwrangling0.sort.o $L2R2_GENOME_DIR/HiFi_L2R2_genome_ALL.sort.tsv | cut -f 2,3,4,5,7,8,9 > $L2R1_L2R2_INTEGRATE_DIR/HiFi_L2R2_genome_spatial.txt
