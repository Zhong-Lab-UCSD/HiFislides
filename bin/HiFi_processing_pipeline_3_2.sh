...
# awk '{A[$1]++}END{for(i in A)print i,A[i]}' $L2R1_L2R2_INTEGRATE_DIR/HiFi_L2R2_genome_spatial.final.$ROI_label.txt | awk -v OFS='\t' '{print $1, $2, $2/10000}' > $L2R1_L2R2_INTEGRATE_DIR/tile_spot_number_table.$ROI_label.txt

# # Calculate total gene expression level in each tile
# awk '{a[$1]+=$7}END{for(i in a) print i,a[i]}' $L2R1_L2R2_INTEGRATE_DIR/HiFi_L2R2_genome_spatial.final.$ROI_label.txt > $L2R1_L2R2_INTEGRATE_DIR/tile_gene_expression_table.$ROI_label.txt
